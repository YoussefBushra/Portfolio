    ...
    <article className="panel p-5 md:p-7">
      ...
      {role.metrics?.length ? (
        <div className="mt-5 flex flex-wrap gap-x-6 gap-y-3">
          {role.metrics.map((m) => (
            <div
              key={m.label}
              className="rounded border border-line bg-surface-2/50 px-3.5 py-2"
            >
              <div className="num font-display text-lg font-semibold leading-none tracking-tight text-accent-text">
                {m.value}
              </div>
      ...
      <ul className="mt-6 grid gap-x-10 gap-y-2.5 lg:grid-cols-2">
        {role.highlights.map((h) => (
          <li
            key={h}
            className="border-l border-accent/30 pl-3.5 text-[13px] leading-[1.6] text-muted"
          >
            {h}
          </li>
        ))}
