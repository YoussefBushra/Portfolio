...
/**
 * Profile links as glass icon buttons — the compact, tappable treatment used
 * on phones, where the desktop's labelled list would be verbose. Falls back to
 * the label's initial for any social without a bundled glyph.
 */
...
export function SocialIcons({ from, className = "" }: { from: string; className?: string }) {
  return (
    <ul className={`flex items-center gap-2.5 ${className}`}>
      {profile.socials.map((s) => {
        const glyph = GLYPHS[s.label.toLowerCase()];
        return (
          <li key={s.label}>
            <a
              href={s.href}
            ...
            >
              {glyph ? (
                ...
                <span className="text-sm font-semibold">{s.label[0]}</span>
              )}
            </a>
          </li>
        );
