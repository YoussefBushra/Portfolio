  ...
  return (
    <Section id="contact" className="py-24 md:py-32">
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-[0.85fr_1.15fr]">
                ...
                <li key={s.label}>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-surface-2/60 font-mono text-xs text-accent transition-colors group-hover:border-accent/50">
        ...
        <RevealOnScroll>
          {status === "success" ? (
            <div className="surface flex h-full flex-col items-center justify-center p-10 text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-full border border-ok/40 bg-ok/10 text-2xl text-ok">
                ✓
              </div>
              <h3 className="mt-5 text-lg font-bold text-text">Message sent</h3>
              <p className="mt-2 max-w-sm text-sm text-muted">
                Thanks for reaching out — your message is on its way. I&apos;ll
            ...
            <form onSubmit={handleSubmit} noValidate className="surface p-6 sm:p-7">
              ...
              {status === "error" ? (
                <p
                  role="alert"
                  className="mt-4 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 font-mono text-xs text-red-400"
                >
                  {errorMsg}
                </p>
              ) : null}
              ...
              <button
                ...
                className="btn-primary group mt-5 w-full justify-center disabled:cursor-not-allowed disabled:opacity-70"
              >
                {status === "submitting" ? (
                  <>
                    <span className="h-4 w-4 animate-spin rounded-full border-2 border-[#04121f]/40 border-t-[#04121f]" />
                    sending…
                  </>
                ) : (
                  <>
